      ...
      <div class="plans-wrapper">
        ...
        <div class="plan-card" v-else v-for="plan in filteredPlans" :key="plan.id">
          ...
          <div class="card-header">
            ...
            <!-- 推荐购买标签（蓝色），按 API 返回顺序（1 基）命中 -->
            <div
              class="card-badge glassmorphism recommend-badge"
              v-if="isRecommendedBadge(plan)"
            >
              <IconBox :size="16" class="badge-icon" />
              <span>{{ SHOP_CONFIG.recommendedBadgeText || '推荐购买' }}</span>
            </div>
          ...
          <div class="card-body">
            ...
            <div class="plan-features">
              ...
              <!-- HTML/Markdown 内容：统一经 Markdown 渲染并净化输出 -->

              <div v-else class="html-content" v-html="renderPlanContent(plan.content)"></div>
...
export default {
  ...
  setup() {
    ...
    const renderPlanContent = (raw) => {
      if (!raw) return '';
      try {
        // 先尝试当作 Markdown 渲染
        const rendered = md.render(String(raw));
    ...
    // 推荐购买徽章：按 API 返回顺序（1 基）命中
    const isRecommendedBadge = (plan) => {
      try {
        if (!Array.isArray(plans.value) || plans.value.length === 0) return false;
        const index1Based = plans.value.findIndex(p => p.id === plan.id) + 1;
        if (index1Based <= 0) return false;
        const list = Array.isArray(SHOP_CONFIG.recommendedPlanIndices) ? SHOP_CONFIG.recommendedPlanIndices : [];
        return list.includes(index1Based);
      } catch (_) {
        return false;
      }
    };
    ...
    .card-header {
      ...
      .card-badge {
        ...
        &.glassmorphism {
          ...
          &.recommend-badge {
            background-color: rgba(53, 92, 194, 0.15);
            border-color: rgba(53, 92, 194, 0.12);
            color: #355cc2;
          }
        }
